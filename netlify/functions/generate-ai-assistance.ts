...
// The Gemini API key is accessed securely from environment variables on the server.
// This must be set in your Netlify project's settings.
const ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });

const generatePrompt = (policy: Policy, userPrompt: string): string => {
...
const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    const { policy, userPrompt } = JSON.parse(event.body || '{}');

    if (!policy || !userPrompt) {
      return { statusCode: 400, body: 'Missing policy or userPrompt in request body' };
    }

    const prompt = generatePrompt(policy, userPrompt);
    ...
    const htmlContent = htmlStartIndex !== -1 ? rawText.substring(htmlStartIndex) : rawText;
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "An error occurred while generating the AI response." }),
    };
  }
};
